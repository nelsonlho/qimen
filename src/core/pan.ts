...
/** 鳴法九神:陽遁順飛之序 */
const GODS_MING_YANG = ['值符', '騰蛇', '太陰', '六合', '勾陳', '太常', '朱雀', '九地', '九天']
/** 鳴法九神:陰遁逆飛之序 */
const GODS_MING_YIN = ['值符', '騰蛇', '太陰', '六合', '白虎', '太常', '玄武', '九地', '九天']
...
export const DOOR_HOME: Record<string, number> = {
  休門: 1, 死門: 2, 傷門: 3, 杜門: 4, 中門: 5, 開門: 6, 驚門: 7, 生門: 8, 景門: 9,
}
...
export function computeChart(input: DateParts, options: QimenOptions = {}): Chart {
  ...
  const isFly = plateStyle !== '轉盤' // 飛盤/鳴法:中五入盤不寄
  ...
  const zhiShiDoor =
    fuPalaceRaw === 5
      ? plateStyle === '鳴法'
        ? '中門'
        : DOOR_BY_PALACE[2]
      : DOOR_BY_PALACE[fuPalaceRaw]

  // 時干(甲用旬首儀)所在地盤宮 = 值符星落宮;飛盤/鳴法中五入盤不寄
  ...
  const earthGods: Record<number, string> = {}

  if (plateStyle === '鳴法') {
    // --- 鳴法:星、儀恆順飛,值符星攜本宮干加時干宮 ---
    for (let i = 0; i < 9; i++) {
      const src = wrap9(fuPalaceRaw + i)
      const dest = wrap9(fuTarget + i)
    ...
    for (let i = 0; i < 9; i++) {
      const src = wrap9(fuPalaceRaw + i)
      const dest = wrap9(landRaw + i)
      doors[dest] = DOOR_MING[src]
    }
    // 九神兩套:天盤自值符星落宮、地盤自值符原宮,皆陽順陰逆,序各異
    const seq = ju.dun === '陽' ? GODS_MING_YANG : GODS_MING_YIN
    for (let i = 0; i < 9; i++) {
      const sky = ju.dun === '陽' ? wrap9(fuTarget + i) : wrap9(fuTarget - i)
      const earth = ju.dun === '陽' ? wrap9(fuPalaceRaw + i) : wrap9(fuPalaceRaw - i)
      gods[sky] = seq[i]
      earthGods[earth] = seq[i]
    }
  } else if (isFly) {
  ...
  for (let p = 1; p <= 9; p++) {
    ...
    palaces.push({
      ...
      earthGod: plateStyle === '鳴法' ? earthGods[p] ?? null : null,
